...
Europe stays in °C (including EGLC). America defaults to °F. Unit toggle is available on every station. US biases in `data/bias.json` are annual °F means converted to °C internally. HRRR/HRDPS rows use the seamless bias as a labeled proxy.

Compare-all: Europe keeps the existing EU set. America uses `gfs_hrrr,gfs_seamless,gem_seamless,gem_hrdps_continental,icon_seamless,ecmwf_ifs025,ukmo_seamless`.
...
Open [http://localhost:3000](http://localhost:3000). Query params: `?s=EHAM&d=today`, `?s=KHOU&d=today`, or `?s=KLGA&d=tomorrow&all=1`.
